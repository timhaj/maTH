...
template<typename T>
void merge(std::vector<T>& arr, int left, int mid, int right, bool isAscending) {
    int n1 = mid - left + 1;
    int n2 = right - mid;
    ...
    int i = 0, j = 0, k = left;
    while(i < n1 && j < n2) {
        if((L[i] <= R[j] && isAscending) || (L[i] >= R[j] && !isAscending)) {
            arr[k] = L[i];
            i++;
        } else {
            arr[k] = R[j];
...
template<typename T>
void mergeSort(std::vector<T>& arr, int left, int right, bool isAscending) {
    if(left < right) {
...
template<typename T>
void mergeSort(std::vector<T>& arr, bool isAscending) {
...
void countingSortByDigit(std::vector<int>& arr, int exp, bool isAscending) {
...
void radixSort(std::vector<int>& arr, bool isAscending) {
    int maxVal = arr[0];
    for (int i = 1; i < arr.size(); i++) {
        if (arr[i] > maxVal) {
            maxVal = arr[i];
    ...
    for (int exp = 1; maxVal / exp > 0; exp *= 10) {
        countingSortByDigit(arr, exp, isAscending);
    }
}
